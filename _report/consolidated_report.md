...
### SEC-004: ID生成における衝突耐性・予測困難性の不足 (Severity: High)

**内容**: `internal/service/id_generator.go` において `math/rand/v2` によるID生成が行われています。

**リスク**: `math/rand` は暗号学的に安全な乱数生成器ではないため、IDの衝突や予測が可能になる恐れがあります。これにより、データの上書きや不正アクセスのリスクが生じます。

**推奨対策**: UUID v4 や CUID、`crypto/rand` を用いたID生成に置き換えてください。
...
### ARCH-002: ドメイン貧血症 (Anemic Domain Model) (Severity: Medium)

**内容**: `internal/dto`, `internal/service` 全般において、データ構造 (`dto`) とビジネスロジック (`service`) が完全に分離されています。

**リスク**: バリデーションルールやデータの加工ロジックがサービス層に散在しており、データの整合性を保証するのが困難です。

**推奨対策**: `dto` はあくまでDBやJSONとの入出力用とし、ビジネスロジックを持つ「ドメインモデル」を導入してください。データソースからの変換処理やバリデーションはドメインモデルの責務として実装してください。
...
| ARCH-002 | アーキテクチャ | Medium | ドメイン貧血症 |
